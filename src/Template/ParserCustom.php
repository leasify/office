...
class ParserCustom
{
    ...
    protected function calculateDataSchema(array &$values, array &$data, array &$mergeCells, SchemaMapper &$schema): array
    {
        ...
        foreach ($values as $row => $columns) {
            ...
            foreach ($columns as $column => $value) {
                if (!$additionRows && $this->hasContractMarker($value)) {
                    $additionRows = $this->getHighestRow($dataKeys, $value);
                }
     ...
     * @return bool
     */
    private function hasContractMarker(?string $value): bool
    {
        if (!$value) return false;
        return str_contains($value, 'contracts.');
    }
     ...
     * @return bool
     */
    private function getHighestRow(array $dataKeys, ?string $value): int
    {
        $qty = 0;
        $string = str_replace(array('[', ']'), '', $value);

        while (in_array(preg_replace('#\.\*(\.|\])#', ".{$qty}.", $string), $dataKeys)) {
            $qty++;
        }
